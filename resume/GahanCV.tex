...
\\ & Terraform, Docker, Docker Compose, Jenkins, GiT 
\\
Cloud Technologies & GCP, AWS, Azure, Vertex AI, Digital Ocean, JFrog Artifactory,
\\ & TeamCity, JIRA, AWS Sage Maker
\\
...
\textbf{Description}: Modernized enterprise workflows, repository management, and code review processes using multi-tool compatible Agentic AI and GenAI architectures (Claude, Cursor).
\begin{itemize}
    \item Developed robust tools, automated git hooks, and modular agents; implemented security compliance and token spend restrictions.
\end{itemize}

{\bf Context-Aware Task Routing \& Triage with LangGraph} \hfill {2025 - Present}\\
\textbf{Description}: Led team sprint goals to achieve automated context-aware task/issue routing and resolution using graph-based LLMs.
\begin{itemize}
    \item Designed smart routing using LangGraph and semantic FAISS indexes, integrating OpenAI, Vertex AI, and local LLM APIs.
    \item Built connectors for GitHub, Jira, and historical systems, with a persona counselor model for summarizing and adapting findings.
\end{itemize}

{\bf DevOps Management for Cluster Validation} \hfill {2025 - Present}\\
\textbf{Description}: Engineered CI pipeline configurations for validation of heterogeneous computing systems and log collection.
\begin{itemize}
    \item Developed automated telemetry log shipping to S3 buckets with integrated cloud network security controls and compliance.
\end{itemize}
