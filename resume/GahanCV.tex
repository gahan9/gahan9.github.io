...
{Senior Software Engineer with 8+ years of experience, currently at AMD India Pvt Ltd managing AI/ML workload execution on HPC GPUs. Strong command of Python, HPC, UEFI firmware, DevOps, and cluster observability (Prometheus, Grafana, Slurm). Experience spanning Agentic AI, knowledge-base design, and CI/CD for large-scale GPU clusters. Open-source contributor focused on practical system software and parallel computing.}
...
\\
Frameworks & Tensorflow, Pytorch, LangGraph, FAISS, TensorRT, Pytest, Edk2, PyCUDA,
\\ & Pandas, UEFI, Selenium, Redfish
\\
Architecture Design & Parallel Programming, ROCm, HIP, Cluster Observability, Agentic AI
\\
Tools and Services & GitHub Actions, Claude, Cursor, LLM APIs, Restful APIs, Teamcity,
\\ & Terraform, Docker, Docker Compose, Jenkins, Git, Prometheus, Grafana, Slurm
\\
Cloud Technologies & GCP, AWS (S3, EC2, SageMaker), Azure, Vertex AI, Digital Ocean,
\\ & JFrog Artifactory, TeamCity, JIRA
\\
Observability \& Ops & Telemetry, Log Shipping to S3, Observability Agents, Cluster Monitoring
\\
System Level IP & RAS, Compute Partitioning, Boot time optimization, CPER
\\
\fi
...
\\ & Docker, Docker Compose, Jenkins, TeamCity, GiT, Claude/Cursor Hooks
\\
Cloud Technologies & AWS EC2, AWS S3, AWS Cloudwatch, VPC Networking, AWS Sage Maker, 
\\ & AWS Lambda, AWS Cloudfront, ECR, ECS, EKS, JFrog Artifactory, Azure OpenAI,
...
\fi
    \item Manage cluster observability and telemetry across large GPU networks using Prometheus, Grafana, and Slurm; automate deployment of observability runners and agents.
    ...
    \item Automate cluster validation CI with secure telemetry log shipping to AWS S3.
    \item Conduct functional and performance benchmarking of HPC GPUs with test suites aligned to industry standards (TensorFlow, PyTorch, GEMM).
    \item Define load-balanced GPU performance criteria covering thermal limits, power, memory usage, and core utilization.
    \item Document RAS injection procedures for MI2XX and MI3XX platforms; cover partitioning, SMI, HIP profiling, and parallel execution.
\if\profile0
    \item Collaborate with engineers and researchers on HPC and Agentic AI tooling.
\fi
\end{itemize}
...
\textbf{Description}: Cluster management and observability for large-scale GPU networks at AMD.
\begin{itemize}
    \item Integrate industry-standard telemetry stacks (Prometheus, Grafana, Slurm) for cluster network monitoring.
    \item Auto-deploy observability runners and agents across large cluster fleets.
    \item Automate secure log upload and telemetry shipping to AWS S3 with CI for cluster validation.
\end{itemize}

{\bf Obsidian Agentic Memory Network} \hfill {2025 - Present}\\
\textbf{Description}: Agentic design for memory index, knowledge base, and skills sync readable by both humans and agents.
\begin{itemize}
    \item Deliver Obsidian-based agentic memory network with indexed knowledge base and auto-synced skills for optimal human and agent consumption.
\end{itemize}

{\bf Agentic AI for Software Engineering Modernization} \hfill {2025 - Present}\\
\textbf{Description}: Modernized enterprise workflows, repository management, and code review using multi-tool Agentic AI (Claude, Cursor).
\begin{itemize}
    \item Built tools, automated git hooks, and modular agents with security compliance and token spend controls.
\end{itemize}

{\bf Context-Aware Task Routing \& Triage with LangGraph} \hfill {2025 - Present}\\
\textbf{Description}: Context-aware task/issue routing and resolution using graph-based LLMs.
\begin{itemize}
    \item Designed routing with LangGraph and FAISS semantic indexes; integrated OpenAI, Vertex AI, and local LLM APIs.
    \item Built connectors for GitHub, Jira, and historical systems with a persona counselor for summarization.
\end{itemize}
